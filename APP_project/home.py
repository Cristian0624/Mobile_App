...
class ReminderCard(BoxLayout):
    ...
    def __init__(self, reminder_data, **kwargs):
        # Adjust card height to fix layout issues
        ...
        self.spacing = dp(5)
        self.reminder_data = reminder_data  # Store reminder_data as an instance variable
    ...
    def _update_rect(self, *args):
        ...
        if hasattr(self, 'title_label'):
            self.title_label.text = self.reminder_data.get('medication_name', 'Medication')
            
    def _create_ui_elements(self):
        """Create all UI elements for the reminder card"""
        ...
        title = Label(
            text=self.reminder_data.get('medication_name', 'Medication'),
            font_size=dp(20),
            bold=True,
            color=(0.2, 0.2, 0.2, 1),
            halign='left',
        ...
        dosage_label = Label(
            text=f"{tr('dosage')}: {self.reminder_data.get('dosage', '1')} {self.reminder_data.get('dosage_unit', tr('pill'))}",
            font_size=dp(16),
            color=(0.3, 0.3, 0.3, 1),
            halign='left',
            valign='middle',
        ...
        frequency_label = Label(
            text=f"{tr('frequency')}: {self.reminder_data.get('frequency', tr('once_daily'))}",
            font_size=dp(16),
            color=(0.3, 0.3, 0.3, 1),
            halign='left',
            valign='middle',
        ...
        buttons_layout = self._create_buttons_layout(self.reminder_data)
    ...
    def _create_take_button(self):
        ...
        self.take_button = take_btn
        
        # Initialize cooldown label and progress circle
        self.cooldown_label = Label(
            text=tr('ready'),
            font_size=dp(14),
            color=(1, 1, 1, 1),
        ...
        with take_btn_container.canvas.before:
            Color(0.9, 0.9, 0.9, 1)  # Light gray background
            self.circle_bg = Ellipse(size=(dp(60), dp(60)))
            self.circle_progress_color = Color(0.2, 0.8, 0.2, 1)  # Green for progress
            self.circle_progress = Line(
                circle=(0, 0, dp(30)),
                width=dp(4),
                close=True
            )
        
        # Position the progress circle
        def update_circle_pos(instance, value):
            self.circle_bg.pos = instance.center_x - dp(30), instance.center_y - dp(30)
            self.circle_progress.circle = (
                instance.center_x, 
                instance.center_y, 
                dp(28),  # Slightly smaller than background
                0,  # Start angle
                360  # End angle (full circle)
            )
        
        take_btn_container.bind(pos=update_circle_pos, size=update_circle_pos)
        
        # Add cooldown label on top of the button
        take_btn_container.add_widget(self.cooldown_label)
        
        # Initial check of cooldown status
        self.check_cooldown_status()
        
        return take_btn_container
        
    def check_cooldown_status(self, dt=None):
        """Check and update the cooldown status"""
        if not hasattr(self, 'next_dose_time') or self.next_dose_time is None:
            # No next dose time set, medication is ready to take
            self.cooldown_label.text = tr('ready')
            if hasattr(self, 'circle_progress_color'):
                self.circle_progress_color.rgba = (0.2, 0.8, 0.2, 1)  # Green for ready
            if hasattr(self, 'circle_progress'):
                self.circle_progress.angle_end = 360  # Full circle
            if hasattr(self, 'take_button'):
        ...
        if hasattr(self, 'circle_progress_color'):
            self.circle_progress_color.rgba = (0.2, 0.4, 0.8, 1)  # Blue for cooldown
    ...
    def take_dose(self):
        ...
        if hasattr(self, 'take_button') and self.take_button.disabled:
        ...
        if hasattr(self, 'take_button'):
            original_color = self.take_button.bg_color
            self.take_button.bg_color = (0.3, 1.0, 0.3, 1)  # Bright green for confirmation

            # Schedule restoration of original color after brief delay
            def restore_color(dt):
                if hasattr(self, 'take_button'):
                    self.take_button.bg_color = original_color
        ...
        now = datetime.now()
        self.next_dose_time = now + timedelta(minutes=cooldown_minutes)
        ...
        if self.reminder_manager:
            ...
            self.reminder_manager.update_next_time(self.reminder_id, self.next_dose_time)
        ...
        # Schedule next update in 1 second
        Clock.schedule_once(self.check_cooldown_status, 1)
...
class HomeScreen(Screen):
    ...
    def _create_take_button(self):
        """Create the take now button with circular countdown"""
        # Main container for the take button and cooldown indicator
        take_btn_container = BoxLayout(
            orientation='horizontal',
            size_hint=(1, None),
            height=dp(100),  # Increased height to accommodate the circle
            padding=[dp(5), dp(5), dp(5), dp(5)]
        )
        
        # Left side - Take button
        left_side = BoxLayout(
            orientation='vertical',
            size_hint=(0.7, 1),
            padding=[0, 0, dp(10), 0]
        ...
        take_btn.bind(on_release=lambda x: self.take_dose())
        self.take_button = take_btn  # Store reference for later updates
        
        left_side.add_widget(take_btn)
        
        # Right side - Cooldown indicator
        right_side = BoxLayout(
            orientation='vertical',
            size_hint=(0.3, 1)
        )
        
        # Container for the cooldown circle
        ...
        take_btn_container.add_widget(left_side)
        take_btn_container.add_widget(right_side)
